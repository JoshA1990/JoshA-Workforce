...
Raw_Data_y1 <- read_csv("Staff in Post - Workforce 202208 extracted Oct 22.csv")
Raw_Data_y2 <- read_csv("Staff in Post - Workforce 202308 extracted Oct 23.csv")
...
#export to shared area with today's date
#write.csv(long_format, paste("C:/Users/Josh.Andrews/OneDrive - Department of Health and Social Care/Nurse Data/Outputs/",str_replace_all(Sys.Date(),"-",""),"joiners_leavers_dashboard",long_format$Date_from[1],".csv"))

#export to shared area with today's date
#write.csv(Reasonleaving, paste("C:/Users/Josh.Andrews/OneDrive - Department of Health and Social Care/Nurse Data/Outputs/",str_replace_all(Sys.Date(),"-",""),"reason_for_leaving",long_format$Date_from[1],".csv"))

#export to shared area with today's date
#write.csv(Country, paste("C:/Users/Josh.Andrews/OneDrive - Department of Health and Social Care/Nurse Data/Outputs/",str_replace_all(Sys.Date(),"-",""),"joiners_by_country",long_format$Date_from[1],".csv"))

#export to shared area with today's date
#write.csv(Recruitment_source_pivot, paste("C:/Users/CSmith7/Data/Monthly Reporting/",str_replace_all(Sys.Date(),"-",""),"joiners_recruitment",long_format$Date_from[1],".csv"))

#export to shared area with today's date
#write.csv(Recruitment_source_breakdown_pivot, paste("C:/Users/CSmith7/Data/Monthly Reporting/",str_replace_all(Sys.Date(),"-",""),"joiners_recruitment_breakdown",long_format$Date_from[1],".csv"))

#################Checks#################################
#check '3146291'- this is bank staff becoming permanent

#test nationality unknowns
#Data %>% filter(is.na(Nationality_y1) == TRUE & Staff_group_y2 %in% c("Nurse"))

#Data %>%
#  filter(is.na(Nationality_y1) == TRUE & is.na(Nationality_y2) == FALSE & Staff_group_y2 %in% c("Nurse") & Status_y2 %in% c("Active") & NHSD_trust_or_CCG_y2 == "1") %>%
#  select(Unique_Nhs_Identifier,Nationality_y1,Nationality_y2)
...
#testing quaterly vs annual joiners/ leavers
#test <- Data %>% filter (non_active_to_active>0)

#test %>% summarise (sum(non_active_to_active))

#active to inactive check
#Data %>% filter(non_active_to_active>0) %>% group_by(Status_orig_y1,Status_orig_y2) %>% summarise (sum(Contracted_Wte_y2))
#Data %>% filter(active_to_non_active<0) %>% group_by(Status_orig_y1,Status_orig_y2) %>% summarise (sum(Contracted_Wte_y2))

#load multiple files code (too slow)
#setwd("C:/R/20200928 Nurse data - ESR/R run")

#file_list <- list.files()
#occ_check <- data.table()

#for (i in file_list) {
#  Raw_Data_midA <- fread(i)
#  occ_check <- bind_rows(occ_check,Raw_Data_midA)
#  rm(Raw_Data_midA)
#}
summary(data)
